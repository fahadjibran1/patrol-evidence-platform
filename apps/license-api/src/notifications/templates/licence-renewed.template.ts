...
export function renderLicenceRenewedTemplate(
  ...
  const bodyHtml = `
    ...
    <table role="presentation" cellspacing="0" cellpadding="0" style="width:100%;border-collapse:collapse;margin:0 0 20px;">
      ...
      <tr><td style="padding:8px 0;color:#64748b;">New expiry</td><td style="padding:8px 0;font-weight:600;">${escapeHtml(data.expiresAtDisplay)}</td></tr>
      ${featuresRow}
    </table>
    <p style="margin:0 0 16px;">Your updated licence file is attached as: <strong>${escapeHtml(data.attachmentFilename)}</strong></p>
  ...
  const bodyText = [
    ...
    featuresText ? '' : null,
    `Your updated licence file is attached as:\n${data.attachmentFilename}`,
    '',
    'Activation:',
